...
def genDataPlant(num_data, num_features, grid, ep_type, deg=1, noise_width=0, seed=135):
    ...
    planted_good = (1, 4, 9, 16, 24, 31, 36, 39)
    planted_bad = (0, 3, 8, 15, 23, 30, 35, 38)
    if type(deg) is not int:
        raise ValueError("deg = {} should be int.".format(deg))
    if deg <= 0:
        raise ValueError("deg = {} should be positive.".format(deg))
    ...
    x_class = rnd.uniform(0, 2, (n, 1)) # np.concatenate((rnd.uniform(0, 2, (n, 1)), np.ones((n,1))), axis = 1)
    ...
    for i in range(n):
        ...
        for j in range(d):
            if j in planted_good:
                ci[0][j] = 2 + f2(x_class[i][0], 0, -4, -.2)
                epsilon[j] = epsilon[j] 
            elif j in planted_bad:
                # ci[0][j] = 2
                ci[0][j] = 1.3 + 0.5*x_class[i][0] 
                epsilon[j] = epsilon[j] 
            else: 
                ci[0][j] += 2.2
        # noise
    ...
    return torch.FloatTensor(x), torch.FloatTensor(c), torch.FloatTensor(c_hat), epsilon
